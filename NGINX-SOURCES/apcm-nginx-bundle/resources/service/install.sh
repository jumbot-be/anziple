...
if [ "$OSTYPE" == "linux-gnu" ]; then
    ...
    installscriptpath=$(cygpath -w "${SCRIPT_DIR}/posh/install-service.ps1")
    #Call the script in a runas admin powershell window.
    powershell.exe  -executionpolicy bypass -Command "Start-Process -FilePath powershell.exe -ArgumentList \"-executionpolicy bypass -File \"\"${installscriptpath}\"\"\" -verb runas -wait"
else
    echo "Unknown operating system. Didn't install service."
fi
